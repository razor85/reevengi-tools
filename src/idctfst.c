...
#else
#    define DEQUANTIZE(coef, quantval) DESCALE((coef), IFAST_SCALE_BITS - PASS1_BITS)
#endif
...
void IDCT(BLOCK* block, int k) {
    ...
    for (i = 0; i < DCTSIZE; i++, ptr++) {
        ...
        if ((ptr[DCTSIZE * 1] | ptr[DCTSIZE * 2] | ptr[DCTSIZE * 3] | ptr[DCTSIZE * 4] |
                ptr[DCTSIZE * 5] | ptr[DCTSIZE * 6] | ptr[DCTSIZE * 7]) == 0) {
            /* AC terms all zero */
            ptr[DCTSIZE * 0] = ptr[DCTSIZE * 1] = ptr[DCTSIZE * 2] = ptr[DCTSIZE * 3] =
                ptr[DCTSIZE * 4] = ptr[DCTSIZE * 5] = ptr[DCTSIZE * 6] = ptr[DCTSIZE * 7] =
                    ptr[DCTSIZE * 0];
